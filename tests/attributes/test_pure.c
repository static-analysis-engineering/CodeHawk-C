/* test pure attribute
   ...
   (the pure attribute guarantees that the function does not cause any
   observable changes to the state of the program).
*/

int f_no_attr(void *p);

int f_attr(void *p) __attribute__ ((__pure__));
